...
fun RegistrationScreen(
    ...
    LaunchedEffect(authState.isSuccess, isGoogleAuthSuccessTrigger) {
        if (authState.isSuccess) {
            if (isGoogleAuthSuccessTrigger) {
                ...
                // isGoogleAuthSuccessTrigger sẽ tự reset khi launcher chạy lại
            }
            // Nếu không phải Google Auth trigger, dialog sẽ tự hiển thị bởi khối if bên dưới
            // (Dialog được đặt bên ngoài LaunchedEffect này)
        }
    }

    // Thêm dialog đăng ký thành công (chỉ hiển thị khi không phải Google Sign-In trigger)
    if (authState.isSuccess && !isGoogleAuthSuccessTrigger) {
        ...
        Dialog(
            onDismissRequest = {
                viewModel.resetState()
                onNavigateToLogin() // Điều hướng đến Login khi dismiss dialog
            },
            ...
            Box(
                ...
                Column(
                    ...
                    AnimatedVisibility(
                        ...
                        Text(
                            text = email, // Hiển thị email đã đăng ký
                    ...
                    Row(
                        ...
                        AnimatedVisibility(
                            ...
                            Button(
                                onClick = {
                                    haptic.performHapticFeedback(HapticFeedbackType.LongPress)
                                    // Lưu email trước khi điều hướng
                                    EmailDataHolder.setEmail(email)
                                    viewModel.resetState() // Reset trạng thái trước khi điều hướng
                                    onNavigateToLogin()
                                },
                                shape = RoundedCornerShape(16.dp),
                                modifier = Modifier.height(56.dp),
    ...
    // Thay thế LaunchedEffect hiện tại với logic mới để kiểm tra trạng thái
    LaunchedEffect(authState.errorMessage, authState.isSuccess) {
        // Chỉ hiển thị lỗi khi không có dialog thành công (isSuccess = false)
        if (!authState.isSuccess && authState.errorMessage != null) {
            // Chỉ hiển thị snackbar cho các lỗi KHÁC lỗi email đã tồn tại
            // vì lỗi đó đã hiển thị trực tiếp trên TextField
            if (authState.errorMessage != "Email này đã được đăng ký." && 
                authState.errorMessage != "Đang kiểm tra email, vui lòng đợi giây lát.") {
                haptic.performHapticFeedback(HapticFeedbackType.LongPress)
